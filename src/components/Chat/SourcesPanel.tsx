    ...
    <details className="mt-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-sm dark:border-emerald-900/30 dark:bg-emerald-900/20">
      ...
      <div className="mt-3 space-y-3">
        {sources.map((source, index) => (
          <article key={`${source.source}-${index}`} className="rounded-xl border border-emerald-100 bg-white p-3 shadow-sm dark:border-emerald-900/30 dark:bg-slate-800">
            ...
            <p className="mt-2 line-clamp-5 whitespace-pre-wrap text-xs leading-relaxed text-slate-600 dark:text-slate-400">{source.content}</p>
          </article>
        ))}
      </div>
    </details>
